...
/** Small interactive map card: tilts on hover, expands into a stylised street map on click. */
export function LocationMap({ location, coordinates, className }: LocationMapProps) {
  ...
  return (
    <motion.div
      ref={containerRef}
      className={cn(
        "relative w-full max-w-full cursor-pointer touch-manipulation select-none",
        className,
      )}
      style={{
        perspective: 1000,
    ...
    >
      ...
      {/* Click hint */}
      <motion.p
        className="absolute -bottom-6 left-1/2 text-[10px] whitespace-nowrap text-muted-foreground"
        style={{ x: "-50%" }}
        initial={{ opacity: 0 }}
        animate={{
