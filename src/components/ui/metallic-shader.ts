// WebGL renderer for the liquid-metal surface of MetallicButton. Ported unchanged from the
// approved Lovable build; browser-only (instantiate it inside an effect).
...
export class MetallicShaderMount {
  ...
  constructor(parent: HTMLElement, uniforms: MetallicShaderUniforms, speed: number) {
    this.parent = parent;
  ...
  private handleResize = () => {
    ...
    const headroom =
      Math.sqrt(DEFAULT_MAX_PIXEL_COUNT) / Math.sqrt(targetPixelWidth * targetPixelHeight);
